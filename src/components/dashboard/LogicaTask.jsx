...
export default function LogicaTask({ onBack, playSound, isMobile }) {
  ...
  return (
    <div className="game-container fade-in" style={{ width: '100%', maxWidth: '1200px', margin: '0 auto', boxSizing: 'border-box' }}>
      ...
      {showGuide && (() => {
        const tabsOrder = ['concept', 'matrix', 'mvp', 'slicer', 'timer'];
        const curIndex = tabsOrder.indexOf(guideTab);
        return (
          <div style={{
            position: 'fixed',
            top: 0,
            ...
            <div className="glass-panel" style={{
              ...
              <div style={{ width: '100%', height: '4px', background: 'rgba(255,255,255,0.05)', borderRadius: '2px', overflow: 'hidden', marginTop: '-12px' }}>
                <div style={{
                  width: `${((curIndex + 1) / tabsOrder.length) * 100}%`,
                  height: '100%',
              ...
              {/* Footer */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '16px', marginTop: 'auto', flexWrap: 'wrap', gap: '12px' }}>
                <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                  いつでもヘッダーの「使い方ガイド」から再確認できます。
                </span>
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  {tabsOrder.map((tabId, idx) => (
                    <button
                      key={tabId}
                      onClick={() => { playSound('click'); setGuideTab(tabId); }}
                      style={{
                        width: '8px',
                        height: '8px',
                        borderRadius: '50%',
                        background: guideTab === tabId ? 'var(--color-amber)' : 'rgba(255,255,255,0.2)',
                        border: 'none',
                        padding: 0,
                        cursor: 'pointer',
                        transition: 'all 0.2s ease'
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  {curIndex > 0 && (
                    <button 
                      onClick={() => { playSound('click'); setGuideTab(tabsOrder[curIndex - 1]); }}
                      className="btn btn-secondary"
                  ...
                  <button 
                    onClick={() => {
                      playSound('click');
                      if (curIndex < tabsOrder.length - 1) {
                        setGuideTab(tabsOrder[curIndex + 1]);
                      } else {
                        setShowGuide(false);
                        localStorage.setItem('logicatask_guide_seen', 'true');
                      }
                  ...
                  >
                    {curIndex < tabsOrder.length - 1 ? '次へ' : '使ってみる'}
                  </button>
                </div>
              </div>
